...
begin
  if current_setting('transaction_isolation') <> 'repeatable read' then
    raise exception 'Welsh fixture corrected V6 league snapshot requires repeatable-read isolation';
  end if;
end;
$$;
...
with expected(
  ...
  ('cardiff', '7cfaf888-773b-43e2-bb9b-f1486ad4823e'::uuid, '554ea736-41f8-4c78-b5e8-0914efce9e08'::uuid, '14295796888948daf053c054771064b465d88bf68d83e84b54214ac32269b15b', '53cd9f290ac7d9cd90f06efe198a21c419a5bcffbb5258341ffb0731eb459d6b'),
  ...
  ('dragons', 'b2e069a6-de05-438f-be7d-1441df6136de'::uuid, '5e0eabe1-fa58-463a-ab29-f4dd213d025d'::uuid, '5e67b42f640487483acc4cbd5a3b041c67711fe57ca7890fc2ca085a5149843b', '49b9a331e8b74bfeb20287aba925490c7a89a0b6f94a930d93613d840e4ac4cb'),
  ...
  select expected.*,
    case when team_key in ('cardiff', 'dragons')
      ...
      else 'injury_lineage_2025-26_2026-08-30_v2' end as cohort_view_version,
    case when team_key in ('cardiff', 'dragons')
      then 'e9bfde5a965bc7921bbe2434088781b68bb837f0ef1b3c1505bd18c8d90a2450'
      else '7f890764273b1a8e389fd8c4b9881f41c76bd82926d8a7af9dc87e79bf17b4ab' end
...
join reporting.aggregate_releases release
  on release.id = bound.release_id and release.status = 'approved'
join reporting.team_release_payloads_v6 payload
  on payload.release_id = bound.release_id
 and payload.team_key = bound.team_key
 and payload.curated_build_id = bound.curated_build_id
 and payload.payload_sha256 = bound.payload_sha256
 ...
 and payload.cohort_view_version = bound.cohort_view_version
 and payload.cohort_evidence_sha256 = bound.cohort_evidence_sha256
join reporting.team_release_injury_lineage_v1 binding
  on binding.release_id = bound.release_id
 and binding.team_key = bound.team_key
 and binding.season = '2025-26'
 and binding.lineage_version_id =
   '2f419706-8c36-58dd-b4cb-e92162e782b8'::uuid
 and binding.candidate_snapshot_version = bound.candidate_snapshot_version
 and binding.member_sha256 = bound.member_sha256;
...
create table analysis.league_dashboard_release_candidate_snapshot_v6_20260831122000 (
  snapshot_version text primary key check (snapshot_version = '20260831122000'),
...
alter table analysis.league_dashboard_release_candidate_snapshot_v6_20260831122000
  enable row level security;
revoke all on analysis.league_dashboard_release_candidate_snapshot_v6_20260831122000
  from public, anon, authenticated, web_reader;

create function analysis.reject_urc_2025_26_welsh_fixture_league_candidate_mutation()
returns trigger language plpgsql set search_path = pg_catalog as $$
begin
  raise exception 'Welsh fixture corrected Year 2 league candidate snapshot is immutable';
end;
$$;

revoke execute on function
  analysis.reject_urc_2025_26_welsh_fixture_league_candidate_mutation()
  from public, anon, authenticated, web_reader;

create trigger urc_2025_26_welsh_fixture_league_candidate_immutable
before update or delete
on analysis.league_dashboard_release_candidate_snapshot_v6_20260831122000
...
with member_state as (
  ...
  select jsonb_build_object(
    ...
    'analysis_window', jsonb_build_object(
      ...
      'basis', 'Exact corrected team release set with reviewed V6 exposure.'
    ),
...
insert into analysis.league_dashboard_release_candidate_snapshot_v6_20260831122000 (
...
select '20260831122000', '2025-26', 'v6',
  'reporting_classification_2025-26_2026-08-31_v3',
  'd9a8d41772ffadd89ad1b40ae3e8494586adc87a5beff372ddfa8307117cc172',
...
do $$
begin
  if (select recorded_injuries from _urc_v6_league_summary) <> 1545
    or (select time_loss_injuries from _urc_v6_league_summary) <> 938
    or (select known_duration_time_loss_injuries from _urc_v6_league_summary) <> 782
    or (select days_lost from _urc_v6_league_summary) <> 20665
    or (select count(*) from _urc_v6_corrected_members
        where cohort_view_version = 'injury_lineage_2025-26_2026-08-31_v3'
          and candidate_snapshot_version = '20260831121000') <> 2
    or (select count(*) from _urc_v6_corrected_members
        where cohort_view_version = 'injury_lineage_2025-26_2026-08-30_v2'
          and candidate_snapshot_version = '20260831101000') <> 14
    ...
    or exists (
      select 1
      from analysis.league_dashboard_release_candidate_snapshot_v6_20260831122000 snapshot
  ...
  then
    raise exception 'Welsh fixture corrected Year 2 league candidate failed its exact gates';
  end if;
end;
$$;
...
select member.team_key, member.season, member.team_release_id,
  member.curated_build_id, snapshot.analysis_version,
...
join analysis.league_dashboard_release_candidate_snapshot_v6_20260831122000 snapshot
  on snapshot.season = members.season
 and snapshot.member_count = members.member_count
 and snapshot.member_set_sha256 = members.member_set_sha256
cross join lateral jsonb_array_elements(snapshot.member_manifest) manifest
join reporting.team_release_payloads_v6 payload
 ...
 and payload.payload_sha256 = manifest ->> 'payload_sha256'
 and payload.payload_sha256 =
   reporting.canonical_jsonb_sha256_v1(payload.dashboard_payload)
where snapshot.snapshot_version = '20260831122000'
  and manifest ->> 'team_key' = member.team_key
  and (manifest ->> 'team_release_id')::uuid = member.team_release_id
  and (manifest ->> 'curated_build_id')::uuid = member.curated_build_id;

revoke all on analysis.league_team_dashboard_release_candidates_analysis_window_v6
  from public, anon, authenticated, web_reader;
...
create table analysis.accepted_release_contracts_v4 (
  ...
  league_candidate_snapshot_version text not null check (
    league_candidate_snapshot_version = '20260831122000'
  ),
...
alter table analysis.accepted_release_contracts_v4 enable row level security;
revoke all on analysis.accepted_release_contracts_v4
  from public, anon, authenticated, web_reader;

create trigger accepted_release_contracts_v4_immutable
before update or delete on analysis.accepted_release_contracts_v4
for each row execute function analysis.reject_accepted_release_contract_mutation();

insert into analysis.accepted_release_contracts_v4 (
  ...
  league_candidate_snapshot_version,
...
select season, analysis_version, classification_view_version,
  ...
  required_scientific_relations, metric_contract,
  evidence_sha256, evidence_locator
from analysis.accepted_release_contracts_v3
...
as $$
  select exists (
    select 1
    from analysis.accepted_release_contracts_v4 contract
    join analysis.league_dashboard_release_candidate_snapshot_v6_20260831122000
      snapshot on snapshot.snapshot_version =
        contract.league_candidate_snapshot_version
...
begin
  if not analysis.release_contract_candidates_available_v1(
    ...
    or has_table_privilege(
      'web_reader', 'analysis.accepted_release_contracts_v4', 'select'
    )
  then
    raise exception 'Welsh fixture league release contract is incomplete or crossed the reader boundary';
  end if;
end;
$$;
