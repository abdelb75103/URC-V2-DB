-- Keep the public V2 snapshot boundary least-privilege and fast.
--
-- The original private visibility gate referenced security-invoker analysis
-- views. PostgreSQL correctly evaluated those nested views as web_reader,
-- which has no analytical-schema grants, so the otherwise approved immutable
-- snapshot failed closed. Candidate equality remains enforced once, at insert,
-- by validate_*_dashboard_v2_candidate triggers. This runtime gate therefore
-- re-verifies only immutable release identities, roster completeness, accepted
-- source release evidence, and denominator identities using direct tables.
    ...
    where m.release_id = c.release_id
      and not exists (
        ...
        where team_context.release_id = m.team_release_id
          ...
          and team_release.status = 'approved'
...
comment on view reporting.latest_approved_dashboard_bundle_v2 is
  'Private immutable V2 bundle gate. Candidate payload equality is enforced by insert triggers; runtime checks use direct release/evidence tables so web_reader needs no analysis grants.';
