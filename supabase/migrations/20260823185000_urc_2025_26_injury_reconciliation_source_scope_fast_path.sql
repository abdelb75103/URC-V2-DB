...
-- Restrict latest-version aggregation to the 2025-26 injury sources
-- before joining active builds. Candidate columns, dashboard bytes and release
...
with active as (
  ...
  select rv.source_row_id, max(rv.version_number) as version_number
  from processing.record_versions rv
  join injury_source_rows source
    on source.source_row_id = rv.source_row_id
  group by rv.source_row_id
), processing as (
  select rv.id as record_version_id, source.team_key, source.file_sha256,
  ...
  join injury_source_rows source on source.source_row_id = rv.source_row_id
