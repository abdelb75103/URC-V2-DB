...
-- Resolve each 2025-26 injury source row through the existing unique
-- (source_row_id, version_number) index. Candidate columns, dashboard bytes
-- and release predicates remain unchanged.
...
with active as (
...
), injury_source_rows as (
  select sr.id as source_row_id, alias.team_key, sf.file_sha256
  ...
  select latest.id as record_version_id, source.team_key, source.file_sha256,
    step.step_version, latest.eligibility_status,
    latest.record_state ->> 'problem_type' as problem_type
  from injury_source_rows source
  cross join lateral (
    select rv.id, rv.step_run_id, rv.eligibility_status, rv.record_state
    from processing.record_versions rv
    where rv.source_row_id = source.source_row_id
    order by rv.version_number desc
    limit 1
  ) latest
  join audit.step_runs step on step.id = latest.step_run_id
