...
**Disclosure 1: an unrecoverable pre-baseline anchor was deleted.** `outputs/urc_final_human_review_2024-25/urc_injury_reviewed_master_with_exclusion_decisions_2024-25_successor_2026-07-23_v4.xlsx`, pinned by ledger `baseline.source_v4` and by `data/2024-25/master/baseline_record.json` at sha256 `b17b77c8...`, is the workbook the v5 baseline was cut from. Phase 5 deleted it, which the plan expressly allowed once v5 was verified, but `outputs/` is Git-ignored so it is not recoverable from history. Consequence on the record: plan acceptance criterion 2, the v4-to-v5 cell-for-cell formatting comparison, can no longer be re-executed against its original target. The v5 baseline itself remains hash-verified, replay remains byte-green, and no published figure depends on the deleted file. Restore from encrypted UCD-managed storage if a copy exists.

**Disclosure 2: decision support for an open item was deleted.** Three files cited by ledger `open_items` source row 1735, which is still `unresolved` (`urc_injury_unknown_fit_resolution_qa_2026-07-24.json`, `urc_injury_unknown_inference_qa_2026-07-24.json`, `urc_injury_unknown_inference_adjudication_2026-07-24.json`), were deleted under group 4 and are likewise not recoverable from Git. Group 4's condition, that step content be consolidated into the ledger, is satisfied for the operative step decisions, which is why replay still reproduces the inclusion CSV byte-for-byte, but it is not satisfied for open-item decision support: nothing in `steps[*].entries` reproduces that QA material. Row 1735 and row 210 therefore remain unadjudicated with thinner evidence than before. Restore from encrypted UCD-managed storage before adjudicating row 1735 if a copy exists. The annotation cannot be added inside the ledger itself without re-pinning its checksum in three places including a live database row, which is Abdel's call, so it is recorded here and in the deletion manifest instead.
...
**Still open: plan acceptance criterion 4.** The restructure plan requires that a small edit reach the website preview through one command plus one review. It does not: the routine loop is one command (`tools/replay.py`) plus one review for the data layer, and reaching the website additionally needs the lineage load, a `release-league` preflight, Abdel's recorded yes, promotion, and `export-team-dashboards`. That is not an oversight in the implementation so much as a collision between the criterion and the binding rule in `AGENTS.md` that every live database write and every release promotion needs Abdel's explicit approval of the exact hosted target. Closing it means either a development-only local preview command that stops short of any live write, or a recorded amendment to the criterion. Awaiting Abdel's decision; the plan status line records the same. Raised by the independent `gpt-5.6-sol` acceptance review on 2026-07-25, which returned thumbs down on this criterion alone.
