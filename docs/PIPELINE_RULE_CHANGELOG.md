...
Status: candidate data accepted; aggregate reporting successor prepared for the existing private-review dashboard. The additive migration `20260915120000_urc_2025_26_candidate_reporting_release.sql` copies the reviewed 2025-26 candidate aggregates into private immutable reporting snapshots and repoints only the 2025-26 V8 dashboard, V5 comparison and V2 cache-token readers. It retains the exact 2024-25 season and the prior 2025-26 V6 immutable release. This is a documented aggregate reporting exception to the V6 curated-build promotion path: the candidate was assembled from the six registered source replacements and the versioned V15 calculator, rather than an active curated-build bundle. Its own unique release ID, reviewed payload SHA-256, source and mapping hashes, rule version, code commit, prior V6 release ID and prior V6 payload SHA-256 are recorded in private release context. It does not claim to replace the retained V6 source release.

Release preflight requires the approved database identity, exact candidate provenance and aggregate hashes, unchanged served predecessor hashes, sixteen team snapshots, one league snapshot, all comparisons, zero IRFU Unknown family rows and the restricted-reader attestation. Reader definitions are retained privately for rollback. The corrective rollback script restores all five predecessor reader definitions and fails unless the former served team and league aggregate hashes still match; this prevents a later dynamic predecessor drift from silently changing rollback bytes. Both release-only and release-plus-rollback rehearsals execute in a rolled-back transaction. The candidate publication changes the 2025-26 data shown on the existing private-review site, not the audience or access controls; team/public access remains subject to `docs/ACCESS_RESTORATION_GATE.md`.
