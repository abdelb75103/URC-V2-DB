...
## 2026-09-15: Six-source 2025-26 intake and aggregate reporting successor

Status: `applied-and-verified-private-review-reporting-successor` on approved project `eukkvswaxweenovqqgzr`, database `postgres`. Additive migration `20260915120000_urc_2025_26_candidate_reporting_release.sql`, SHA-256 `2390ea68dc6dce972de47c67cd43e9534f257175ee71fe4214b9bdf35782c606`, is applied and checksum-registered. Release `71a05cb3-e11b-42be-bd48-931805c74e75` copies the reviewed 2025-26 candidate aggregates into private immutable reporting snapshots and repoints only the 2025-26 V8 dashboard, V5 comparison and V2 cache-token readers. It retains the exact 2024-25 season and prior 2025-26 V6 immutable release `f1d9c2cc-f70c-4dcc-a18d-3f2dc92d4cfc`. This is a documented aggregate reporting exception to the V6 curated-build promotion path: the candidate was assembled from the six registered source replacements and the versioned V15 calculator, rather than an active curated-build bundle. Its own unique release ID, reviewed payload SHA-256, source and mapping hashes, rule version, code commit, prior V6 release ID and prior V6 payload SHA-256 are recorded in private release context. It does not replace the retained V6 source release.

Release preflight required the approved database identity, exact candidate provenance and aggregate hashes, unchanged served predecessor hashes, sixteen team snapshots, one league snapshot, all comparisons, zero IRFU Unknown family rows and the restricted-reader attestation. Reader definitions are retained privately for rollback. The corrective rollback script restores all five predecessor reader definitions and fails unless the former served team and league aggregate hashes still match; this prevents a later dynamic predecessor drift from silently changing rollback bytes. Release-only, release-plus-rollback and checksum-registration rehearsals passed in rolled-back transactions. Post-release capture proves exact JSON parity with all sixteen 2025-26 candidate team dashboards, the league dashboard and all comparisons, plus exact parity with all sixteen former 2024-25 team dashboards and the former 2024-25 league dashboard. `web_reader` has no direct grant on the private snapshots, reader target attestation is true and all 402 IRFU candidate injury mappings have zero Unknown rows. The 2025-26 served league has 1,560 recorded injuries, 922 Time Loss injuries, 451 illnesses and sixteen source-backed exposure and HSR teams. June 2026 lacks exposure for Benetton, Cardiff, Edinburgh, Lions, Scarlets and Sharks; its official league monthly rates remain null with ten contributors. Publication changes the 2025-26 data shown on the existing private-review site, not the audience or access controls; team/public access remains subject to `docs/ACCESS_RESTORATION_GATE.md`.

Rule `urc_intake_candidate_20260915_v2` applies to the four rebuilt IRFU 2025-26 injury/illness sources and the reviewed Benetton/Edinburgh exposure inputs. Abdel authorised intake and delegated adjudication in task `01a0a287-a441-7ba3-bb63-976962cf2f4f`, initially preserving the served dashboard and later authorising main/live promotion. The six exact file hashes and verbatim authority are retained in the ignored V15 intake root. Registration preserves all 13,548 rows and records that root hash in the source-file manifest.

The four IRFU replacements retain all 842 source records, using their reviewed inclusion ledger. Known source anatomy takes precedence over conflicting Orchard prefixes. Exact source diagnoses remain in the source rows. For the 402 included injuries, the candidate-only family mapping reuses 256 inherited accepted diagnoses through exact canonical punctuation/spacing normalisation and maps 139 rows through 69 explicit aliases to existing accepted families. Only seven single-row labels become new candidate identity groups. `Dental Injury` reuses the accepted dental family. The finger joint sprain with fracture pathology remains an exact candidate identity rather than being called a ligament tear or a fracture. Every mapped row retains its Orchard code and pathology evidence; no broad text inference changes the source diagnosis. All four IRFU teams now have zero Unknown diagnosis rows. The other twelve teams retain their effective injury and illness rows, including the September injury-to-illness correction; their 50 Time Loss Unknown diagnoses remain unchanged.
...
The private candidate schema retains source bridges, typed rows, reused versioned SQL calculators, 32 team payloads, two league payloads and 17 comparisons. Active curated builds and release selectors remain unchanged. A development-only local file option renders the candidate through the shared dashboard components. Production cannot select that file.

## 2026-09-15: Local 2025-26 Benetton exposure join and source exceptions

Status: `applied-in-2025-26-private-review-reporting-successor`. Rule `benetton_weekly_join_20260915_v3` applies only to this Benetton 2025-26 submission. Abdel supplied the calendar and source-season interpretation and explicitly requested retention of friendly-match minutes and distances. Its reviewed rows and exposure aggregates now contribute through the six-source V15 reporting successor above, replacing the temporary estimate in the private-review dashboard.
...
Local canonical data, source decisions, monthly totals, QC and review evidence are retained under `data/intake/2025-26/benetton/`. The V15 successor carries this rule version and its source-bound decisions. No automatic carry-forward to another season is authorised.
...
The approved Injury Master rows are Dragons 401, 446, 464, 468, 481, 494, 509, 510, 514, 515, 555 and 556; Ospreys 1608, 1610, 1611, 1622, 1631, 1633, 1645, 1646, 1662, 1666 and 1696; Glasgow 949. Each physical Excel row resolves to the same accepted source row and exact immutable master-row hash in version `2f419706-8c36-58dd-b4cb-e92162e782b8`. The accepted master CSV SHA-256 is `1872fae1f33c32fab6ec5b86a83a8e475f463910a72c76d4546d35bcf7228264`. All 24 remain included, with original source Problem type, classification and duration retained in the private decision evidence.

The decision changes effective reporting Problem type from Injury to Illness. Dragons contributes twelve Time Loss cases and 54 days; Ospreys contributes eleven Time Loss cases and 38 days. Glasgow remains Medical Attention with null database time-loss duration, despite the workbook's zero display. Six rows lack a prior illness exact-label mapping. They retain the exact source labels `Asthma/allergy/hay fever/respiratory`, `Other skin infection not specifically mentioned` and `Migraine` as three identity groups. No broader clinical category is inferred.

The successor uses the existing row-based injury, diagnosis-family, severity, illness and comparison calculations. Its canonical calculator recognises both retained monthly label formats. It preserves suppressed official monthly rates, original metadata, exposure, HSR, the complete 2024-25 payloads and the thirteen unaffected 2025-26 team payloads. V7 readers select new private payload snapshots; V8 dashboards and V5 comparisons inherit that correction. A changed cache token invalidates warm-process results. No source, master, inclusion or immutable release row is updated.

Verified 2025-26 totals moved from 1,545 to 1,521 injuries, 938 to 915 Time Loss injuries, 607 to 606 Medical Attention injuries, and 20,665 to 20,573 injury days. Illnesses moved from 439 to 463, known-duration illness cases from 202 to 225, and illness days from 927 to 1,019. These are record-derived acceptance checks, not substituted headline constants. Exactly 24 injury rows moved to illness; the other 3,183 canonical injury rows across both seasons and all 831 original illness rows are unchanged. Source master and inclusion hashes are unchanged.

All seven live SQL contracts, 34 actual restricted-reader dashboard/report-model paths, 17 comparisons, TypeScript checking, 35 report tests and six focused migration tests passed. The live league Overview, Illnesses, Season Comparison and 13-page report preview show the correction. Four local report renders, 52 pages total, passed text-boundary and visual layout checks. These renders are QA evidence, not cleared sharing deliverables. Full reconciliation and final migration registration evidence are in the saved checkout's `outputs/illness_correction_20260903/` directory.

The actual 2025-26 sharing workbook changed only the 24 column E cells listed above. Its SHA-256 is `60aa2924dafa813a92cc142cead329332778cef94de382cb8bf8382d3702be42`. All other values, formulas, styles, blanks and prior sharing corrections remain intact. The actual 2024-25 sharing workbook is unchanged, SHA-256 `b0e8cb1cc7608be1636e9f13a32d7d89a89e785aa78c32c591b123c693d73372`. Workbook recovery uses `outputs/share_sanity_20260903/illness-correction/current-state-backup-20260903.xlsx` in the saved checkout. Database recovery uses `tools/sql/rollback_urc_2025_26_injury_to_illness_reporting.sql`, retaining every correction record. A transaction-only rehearsal proved exact restoration of all V8 dashboards, canonical rows, illness aggregates and cache tokens before the actual application.

Independent fresh-context Sol extra-high review accepted the targeted correction. It checked 1,173 diagnosis/type-family rows, 1,569 subtypes, 1,049 diagnosis profiles, all 17 comparison structures and 30 unchanged dashboards. Its separate ZIP/XML scan of 89,833 workbook cells confirmed exactly 24 column E value changes and no other value, formula, style or fixture changes.

That review identified a pre-existing monthly-rate numerator/denominator mismatch, outside the approved 24-row correction. `analysis.urc_2025_26_preliminary_monthly_rates_v1` counts all teams' injuries but divides by the available contributor teams' exposure and labels the result contributor-aligned. All ten months are affected, adding 82 Time Loss cases and 2,459 days from team-months outside the denominator. These excesses are identical before and after this correction. September now uses 94 Time Loss injuries over 9,613.7633 hours, although those 14 contributing teams have 86 Time Loss injuries. Before this correction the same mismatch was 98 versus 90. The dashboard and PDF overview sparklines expose this series; the report monthly timeline also derives missing rates from all-team counts and partial exposure. Annual figures and this correction's parity are unaffected. Suppressing incomplete-exposure rates or changing their numerator policy needs a separate decision before an unqualified supervisor-ready verdict. No email, upload, publication or access-control change was made.
...
| Migration | Additive successor `20260902010000_urc_hsr_reporting_v8.sql`, SHA-256 `4a2598034b3715263231468444762eb453ed56a025f36e9e45d651284f8b3fb3`; checksum registration `tools/sql/register_urc_hsr_reporting_v8_migration.sql`; read-only proof `tools/sql/verify_hsr_reporting_v8.sql`. |
...
**Accepted Zebre distance exception.** Exact-locator preflight resolves all 166,207 payload observations once, with no duplicate or unresolved active locator. The local source payload retains pre-correction distances on exactly 976 Zebre 2025-26 rows, while the live accepted build carries the correction released on 31 August. Those source distances sum to 371,668,301,813 metres; the current accepted values sum to 5,919,231.76391 metres. HSR remains bound to the exact source row and uses the existing corrected live denominator. The migration pins this source-to-accepted difference to the exact season, team, row count and matched-pair checksum `8f056d74844db183ccfda9692cbdad4682e3592c6c8e76fb2c7e7c9021952746`; any other mismatch fails closed.

**Comparability and boundary.** HSR definitions, thresholds, units and vendor calibration vary or are unknown, so direct team HSR rankings are not shown. The Zebre source anomaly and the existing accepted correction are disclosed on relevant HSR views. All 32 team-seasons remain in the reader: Benetton and Edinburgh 2025-26 have no included in-season HSR denominator, so their HSR distance and percentage remain null rather than being invented. V8 retains all V7 fields through successor readers and grants `web_reader` select access only to those readers and their target attestation; private HSR tables and helpers remain inaccessible.
...
| Field | Value |
|---|---|
| Status | `applied-and-promoted`: independent Sol xhigh review accepted the exact candidate. The additive migration and checksum registration are live on approved project `eukkvswaxweenovqqgzr`, database `postgres`. Immutable release `0f0def1e-021f-471f-979f-6d73d98859c4` (`urc-2024-25-v5-a80040f6afaa-a1`) is approved and the least-privilege `web_reader` path returns the successor. |
...
**Release verification.** The reviewed preflight SHA-256 is `239cba176e1dc79180b5374f9d55a8278c93ac029a8940179be558038626a753`; member input SHA-256 is `b711f7fa11b9a8bb66244b225a2d5cedba6840d05a8a6b9dac180f85b4070aa2`; league payload SHA-256 is `4517f50bdf03688c087a34062071d97bd635576011e02f6f8ca5d1dc69a156ae`; bundle payload SHA-256 is `a80040f6afaa143d8c4cd57a66adf29119f3aa43e20565bc077bee5589332b34`. The reader serves 63,273 exposure rows, 79,908.343109 hours, 1,662 recorded injuries, 913 time-loss injuries and 17,575 observed days. The only changed team payloads are Cardiff, Dragons, Edinburgh, Glasgow, Ospreys and Scarlets, plus the league aggregate. No deployment or access-control change ran.
...
| Field | Value |
|---|---|
| Status | `team-foundation-live-cutover-reviewed`: migrations `20260830150000` and `20260830155000` are applied and checksum-registered on approved project `eukkvswaxweenovqqgzr`, database `postgres`. The stale league migration `20260830160000` was never applied or registered and has been withdrawn. The V3-bound cutover `20260830170000` passed independent Sol high pre-write review. No V3-bound team or league promotion had run when this update was recorded. |
...
**Source-backed denominator.** Fourteen team exposure files contribute 62,481 included rows and exact accepted hours of `76872.2616717166666666`. Curated builds select each file by its reviewed SHA-256, not by filename alone. The package preserves the candidate decisions, predecessor chain and source-row lineage. No exposure value is imputed inside those files.

**Temporary estimates.** Benetton and Edinburgh have no source-backed exposure rows. Each receives the exact temporary season estimate `5490.8758336940476190`. Fixture-derived match hours remain unchanged and training hours equal the temporary total less those match hours. Monthly exposure and distance remain unavailable. A valid source-backed denominator always takes precedence, so a future reviewed submission retires the reporting overlay without changing source history.
...
| Field | Value |
|---|---|
| Status | `installed-and-verified-private-successor`: independent Sol xhigh pre-write acceptance passed. The additive migration, checksum registration and exact attested delta are live on approved project `eukkvswaxweenovqqgzr`, database `postgres`, as successor `2f419706-8c36-58dd-b4cb-e92162e782b8`. The predecessor remains immutable. No exposure ingest, release, dashboard change or deployment ran. |
| Rule version | `urc_2025_26_injury_review_triage_2026_08_30_v5` |
| Carry-forward | Frozen 2024-25 zero-day precedence, ordinary date subtraction, excluded-row triage and valid Fit For Selection evidence carry forward. Scottish duration tolerance and Zebre static-closing interpretation remain source-specific. |
| Migration | Additive successor `20260830140000_urc_2025_26_injury_review_triage_successor.sql`, SHA-256 `76598d5843072cf1b4673a1aacdaed907874c402cd6fdd88a2956ccf598cc37a`. |
| Evidence | Manifest SHA-256 `7f890764273b1a8e389fd8c4b9881f41c76bd82926d8a7af9dc87e79bf17b4ab`; delta-payload SHA-256 `111328427560503939a66e845d4a6e0fb8fa606f9dbf4a6f508aa0df04cab637`; delta-evidence SHA-256 `f9e8d82998232a2e7e6f7325f319a685546197b4f4c3ff022f366fafa854c78a`. |
| Decision provenance | Abdel Babiker, 30 August 2026. |

**Zero-day precedence.** Usable zero Days Injured is Medical Attention before a source Time Loss display label, matching the frozen 2024-25 successor. A reported zero with a same-day or next-day return remains zero-day Medical Attention. Irish-team Days Injured is the classification authority when no return date was supplied: zero is Medical Attention and a positive value is Time Loss. Missing Irish return dates stay missing. Source display cells remain unchanged and deterministic disagreements remain in evidence lineage.

**Date-duration resolution.** Ordinary Confirmed Return Date minus Date Injured is the elapsed duration and already excludes the injury day. An extra subtraction is not applied. Included non-Scottish disagreements resolve from that elapsed duration. Scottish reported duration is retained when it is within plus or minus one day of the elapsed duration; larger differences resolve from the dates. The zero-day same-day or next-day exception is applied first. Excluded rows are not classification- or date-duration-adjudicated and do not enter the active queue unless eligibility is reconsidered.

**Participation evidence.** For an included row where explicit classification and stronger contrary evidence are absent, a valid Fit For Selection Date later than Date Injured is direct participation-restriction evidence and classifies the record as Time Loss. For included Zebre rows, an audited static closing date with a non-ongoing record is direct restriction evidence even when the formula-driven Days out field is blank. Volatile `TODAY()` closure values remain non-qualifying. Excluded rows receive no new classification decision.

**Boundary.** The successor changes derived 2025-26 duration values, classification evidence and review status. It does not alter source copies, master-row membership, inclusion membership, exposure, rates, release payloads, the dashboard, deployment, or any 2024-25 artefact. The additive migration creates a private v3 lineage successor, clones the immutable v2 predecessor and substitutes only checksum-bound row states. It grants no dashboard reader access.

**Result.** The v5 rebuild retains 2,993 master rows, 1,923 included rows and the source-bundle SHA-256 `6625218eaed1ebb918e02f5d43e162360786ce62ed8020f9f9f0b4e3eeaf9d28`. Against installed predecessor `bab7731d-975b-5d49-a34c-6acc6b0c8c94`, the exact delta contains 1,975 affected evidence rows, 438 changed master row values, 277 changed inclusion row values, 162 changed classifications and 71 changed clinical durations. Inclusion membership is unchanged. The included injury classification is 877 Time Loss and 607 Medical Attention. All included records are 1,083 Time Loss and 840 Medical Attention, with 350 genuine source conflicts retained, zero unclassified and zero active adjudications. The master CSV SHA-256 is `1872fae1f33c32fab6ec5b86a83a8e475f463910a72c76d4546d35bcf7228264`; review workbook `2b5e2243bfc912fac1561789e9327987d058a5543233f068f3bef9928c397670`; inclusion CSV `f45d0fa09e3c1419f9db33e000a12c41e37e0f8f7c049c155970640330a46b33`; classification evidence `36013e4249b0997ef5e3c6067226c61c2d9321dd58825de25000ca528d70d247`.

**Live verification.** The post-write proof matched every payload predecessor and successor state, found no difference outside the accepted delta and reproduced every count above. RLS is enabled on all four v3 tables, four immutability triggers are active and `web_reader` has no access. Existing 2025-26 source-file, source-row, record-version, curated-injury, exposure, denominator and release counts match pre-write exactly. The frozen 2024-25 lineage remains 3,060 master rows, 755 excluded and row-value aggregate SHA-256 `fad2ab34a166804d723744b6f351f5ce0368e98fbfac5b7e61e2f110d592cc05`.

## 2026-08-30: First valid 2025-26 injury source and classification successor

| Field | Value |
|---|---|
| Status | `installed-and-ingested-private-lineage`: independent Sol xhigh acceptance passed for the exact payload bytes. The additive migration and checksum registration are live, and version `bab7731d-975b-5d49-a34c-6acc6b0c8c94` is ingested and verified on approved project `eukkvswaxweenovqqgzr`, database `postgres`. No exposure ingest, release, dashboard change or deployment has run. |
| Rule version | `urc_2025_26_injury_classification_2026_08_29_v2` |
| Carry-forward | `season-specific`: this successor applies only to the corrected 2025-26 injury lineage. It does not inherit row decisions from 2024-25 or define a future season. |
| Migration | `20260829230000_urc_2025_26_valid_injury_ingest.sql`, SHA-256 `f4c5e3986e13a0b3b1a2e4dda6759f1cf476095ecb77e38554d1226936eba62b` |
| Evidence | Manifest SHA-256 `9fc869cff5732becb20db3b7ec2bbd71a619096aaa32895ff0ac726fbf9395c7`; load-payload SHA-256 `7a11596713ce730a4404039a958d8a8c10cca2092115a8fa40d951e22940e8c2`; source-bundle SHA-256 `6625218eaed1ebb918e02f5d43e162360786ce62ed8020f9f9f0b4e3eeaf9d28`; classification-evidence SHA-256 `b074d8cdc21b42c4659a622f20e611545513bc55b7c5e795d0405f1de507ff51`. |

**Classification and duration.** Explicit source classification or direct participation-restriction evidence defines Time Loss or Medical Attention. A zero-day record is Medical Attention when no stronger contradictory evidence exists. Positive clinical duration alone does not define Time Loss, and missing duration is not zero. An open record without unrestricted-participation evidence falls back to Time Loss with null return date and null Time Loss days. Classification, clinical duration, Time Loss days, basis, return evidence and open status remain separate. A source/master display value that was itself derived from days remains in the raw 29-column row but is not relabelled as qualifying source-classification evidence.

**Conflicts and unknowns.** The successor retains source conflicts for adjudication. It does not use a date-duration mismatch to recast an explicit class. The first valid master contains 2,993 genuine rows, with 1,923 included and 1,070 excluded. The separate inclusion layer contains 1,484 injury rows and 439 illness rows. The injury cohort currently reconciles to 923 Time Loss, 548 Medical Attention and 13 unclassified records. Of 170 open Time Loss injuries, 161 lack unrestricted-participation evidence and therefore have null processing return dates and null Time Loss days. The other 9 have checksum-bound Green participation evidence. The evidence layer retains 636 source conflicts and 715 review-required master rows. These retained uncertainties block no private lineage ingest, but they remain outside a released dashboard decision.

**Artefacts and boundary.** The 29-column master CSV SHA-256 is `c8c9839445ac2af1d6e68f85c23c50bac784273387f6f1c7411a4deb62b329bd`; review workbook SHA-256 is `15be3edbe9617f6627e421e6cad470320aa5c5c9e5aed48c92a8225b3c24d643`; separate inclusion CSV SHA-256 is `857240ebf02880ce82e0c58b8907e28f9a58446d6a703abc6179c9ecef238394`. The source bundle uses Scotland v18, including its matched canonical CSVs and audit, and supersedes the blocked v1 consolidation. The output vocabulary is restricted to the frozen 2024-25 workbook values; direct lexical variants are normalised, while labels requiring contextual or clinical inference fall back to a frozen `Unknown`, `N/A or Unknown`, `Other` or blank value with the original retained in the immutable source copy. The workbook retains the frozen two-sheet shape, 151 accepted fixtures, table filter buttons and established review colours. Blue is limited to the 729 included `Confirmed URC match fixture` additions; source-labelled `URC` values are not mislabelled as fixture additions. The approved executor rejects any parameters file whose byte digest differs from the accepted load-payload SHA-256 and records that attestation with the private version row. The database also enforces one valid version for the season. The additive migration grants no dashboard reader access and does not ingest exposure, calculate rates, promote a release, change the dashboard or deploy.

**Live verification.** The post-write proof found zero version, master or inclusion payload mismatches; one loaded version; 2,993 master rows; 1,923 inclusion rows; and the exact 16-team and classification splits stated above. The private lineage tables retain RLS and grant no `web_reader`, anonymous or authenticated read access. Existing 2025-26 source, record-version and curated-injury counts are unchanged. Protected exposure, denominator and release counts match the pre-write proof exactly.

**Frozen prior season.** The 2024-25 review workbook remains SHA-256 `4f1db130f9f5aff23c3473eb2ab64a467f739a0b6ac7e4f170ca0383d9072b73`. Both live proofs pin 3,060 master rows, 755 excluded rows, 2,305 included rows, baseline record SHA-256 `6cbb6d45d6dd181b9bda3a228cf4c86d509060a75f631818726a4748115e0217` and row-value aggregate SHA-256 `fad2ab34a166804d723744b6f351f5ce0368e98fbfac5b7e61e2f110d592cc05`.
...
| Field | Value |
|---|---|
| Status | `applied-and-promoted`: the governed successor and its two narrow release-context contracts are registered, and the complete 16-team bundle is approved on the live URC database. |
...
| Evidence | Classification evidence `docs/evidence/urc_2024-25_classification_monthly_successor_2026-08-26.json`, SHA-256 `2a34003e961b669100b23f6c088e797a1d18171fa720e59582ac70916768aa38`; diagnosis evidence `docs/evidence/urc_2024-25_specific_diagnosis_evidence.json`, SHA-256 `a43ba36a7f67ecd208112d702bcc058de947b00d721399e9e0ad26d23f3ac167` |
| Migration | Successor `20260826100000`, SHA-256 `e25baddbcf5870f61d5bec68b9130ee2a616af99ece32e9317126e401681280c`; release context `20260827150000`, SHA-256 `e3b3538d5da7ccd6c83061293f325054b7a338087386905514bc9e0da0a2fbc3`; correction-aware guard `20260827160000`, SHA-256 `5d088a059bbd43ae0245b1e018287b0fb06ef44383cfc404b7c40579261ff168` |

**Classification and duration.** The correction-aware successor separates final classification from duration. Source-reported and adjudicated Time Loss cases remain Time Loss when duration is null, count in incidence, and remain internally open or ongoing without invented days. Medical Attention and zero-day cases are closed and contribute only to recorded counts; Date Injured is the closure date when available, and missing dates are not imputed. Remaining unclassified cases also contribute only to recorded counts. Severity, mean, median and burden use known-duration Time Loss cases.
...
**Diagnosis and presentation.** Reviewed specific-diagnosis groups replace the derived location and injury-type compound for diagnosis views. The evidence contains 1,660 injury mappings; 392 illnesses are excluded, four correction-aware eligible injuries fall back to internal `unknown`, and front-facing diagnosis lists hide `unknown`. Overview and timeline show Injuries with TL injuries, and Overall incidence with TL incidence. Match and Training overview filters use their own released total and TL values. Other incidence views remain TL incidence. Shared exposure-hour labels round to whole hours while stored values and rates retain full precision.

**Live attestation.** Release `urc-2024-25-v5-15327d9c833f-a3` (`c95aa8e5-59b9-4d4e-bf0c-478d5c95f2ca`) was promoted with reviewer `Abdel Babiker`. The league payload SHA-256 is `2b14179eb8572028cedffb98eaddcc4ee23c14fbc8ae8bf121c3057081223cce`; the complete bundle SHA-256 is `15327d9c833fdee372c7aaebb27a5a2d5d54b9d8e6a3f467af09f307aabf6946`; the 16-team parity export-set SHA-256 is `3a483aa9047331590cb73dd91a1634926bd2ceeca910a88c67d7e7be8f18e40b`. Post-promotion proof through the website's approved database reader found one approved league bundle, sixteen team payloads, 1,662 injuries, 913 TL injuries, 17,575 observed days, paired overall and TL rates for Overall, Match, Training and all ten monthly rows, and injury-only specific-diagnosis profiles.
