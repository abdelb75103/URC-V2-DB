...
| 7. Verify | Run `reconcile-curated` and `verify-analysis-parity` against the exact candidate. For 2025-26, `verify-analysis-parity` selects only the checksum-bound V6 team candidate, validates the strict V6 public shape, and compares the complete reviewed payload using PostgreSQL canonical JSON hashes; it never uses `analysis.*_v1`. Historical seasons retain the existing V1 renderer. | Count/metric reconciliation and candidate parity evidence. | Read-only against the database; no promotion. |
...
After reviewing each team preflight, run `verify-analysis-parity --team <name> --season 2025-26 --dashboard-file <exact-preflight.json>` through `pipeline/run_with_pooler.mjs`. The V6 branch proves the registered migration and local evidence checksums, validates both JSON values against the strict public contract, asks PostgreSQL to hash both the active candidate and reviewed preflight canonically, and compares every public field. Its Git-ignored evidence contains only public aggregate differences and contract hashes. A missing candidate, changed hash, unexpected field, stale evidence identity, or any full-payload difference fails closed. Do not route a Year 2 preflight through the historical `analysis.*_v1` renderer.
