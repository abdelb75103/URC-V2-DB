...
| 7. Verify | For 2025-26, run `verify-analysis-parity` against the exact V6 preflight; `reconcile-curated` remains a historical diagnostic, not a V6 release gate. PostgreSQL hashes the active V6 candidate, while the adjacent preflight manifest binds that hash and release identity to the exact reviewed file bytes. The gate validates the strict public shape and compares every scalar exactly, treating numeric scale alone as equal; it never uses `analysis.*_v1`. Historical seasons retain the existing V1 renderer and tolerance behaviour. | Candidate parity evidence for V6; historical count/metric reconciliation where applicable. | Read-only against the database; no promotion. |
...
V6 team preflight uses `release --season 2025-26 --preflight`. It refuses an uncommitted working tree before any database read, then loads the candidate and full approved predecessor set in one database snapshot. The candidate is read as PostgreSQL `dashboard::text`; preflight preserves that exact text with one terminal newline, and parses ordinary and exact-decimal trees from the same text. Its Git-ignored manifest binds the exact payload, build and automatic immutable predecessor, classification and cohort evidence, required migration version/name/SHA triples, and that single code/dependency provenance snapshot. Promotion accepts neither `--previous-dashboard-file` nor `--restatement-file`; it requires the reviewed preflight and exact `--preflight-reviewer "Abdel Babiker"`. Locked revalidation and insertion cast the same raw candidate text back to `jsonb`, rather than serialising a Python numeric representation. Team promotion writes no public or local dashboard export. Only a completed league closeout writes the league file and full 16-team parity set.

After reviewing each team preflight, run `verify-analysis-parity --team <name> --season 2025-26 --dashboard-file <exact-preflight.json>` through `pipeline/run_with_pooler.mjs`. The V6 branch separately proves the registered database migration checksums and local evidence bytes. In one database snapshot, it loads the active candidate's exact PostgreSQL JSON text, its PostgreSQL canonical hash, and the full approved predecessor set. The adjacent manifest binds that candidate hash, build, tuple, evidence, predecessor and code provenance to the exact reviewed file bytes. Both ordinary payload trees must pass the strict public contract, then exact-decimal trees parsed from the same source bytes compare every public scalar. Integer and floating-point representations with the same numeric value, such as `12` and `12.0`, are equal; any non-zero numeric difference and any boolean-versus-number difference fails. Its Git-ignored evidence contains only public aggregate differences and contract hashes. A missing candidate or manifest, changed hash, unexpected field, stale evidence identity, ambiguous predecessor set, or full-payload difference fails closed. Do not route a Year 2 preflight through the historical `analysis.*_v1` renderer.
