...
Date: 2026-07-24. Status: **all six phases executed 2026-07-24; five of six acceptance criteria met.** Acceptance criterion 4 (a small edit reaches the website preview through one command plus one review) is **NOT met** and is the one open item: no such command exists, and a genuine one-command path to the website would have to cross the live-write and release-promotion approval gates that `AGENTS.md` makes binding. Two ways to close it, awaiting Abdel's decision: build a local, development-only preview command that stops short of any live write (the boundary `AGENTS.md` already contemplates for his private review), or formally amend the criterion to match the approval-gated reality. Do not record this plan as fully accepted until one of those lands. Flagged by the independent `gpt-5.6-sol` acceptance review, 2026-07-25. Phases 0 to 3 and 6 landed first; Phase 4 promoted the restated bundle `urc-2024-25-v4-6f04bd64d2a6-a2` after Abdel's recorded yes on the diff summary; Phase 5 deleted 93 items (354.5 MB) per the manifest below with Abdel's approval. See `docs/PIPELINE_RULE_CHANGELOG.md`, entry "2026-07-24: 2024-25 lineage restatement released, and Phase 5 cleanup executed", for the released figures, the equivalence argument for the performance-only migration, and the recorded deviation on ledger-evidence marking. This document is retained as the plan of record; it is not a live task list.
