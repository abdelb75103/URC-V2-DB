...
Date: 2026-07-24. Status: **COMPLETE. All six phases executed 2026-07-24; all six acceptance criteria met, criterion 4 as amended 2026-07-25.** Criterion 4 was amended on Abdel's decision after the independent `gpt-5.6-sol` acceptance review correctly found the original wording unmet: a single command from edit to website would have to cross the live-write and promotion approval gates that `AGENTS.md` makes binding, so the one-command, one-review property is recorded where it belongs, in the data layer. The amendment, the original wording, and the reasoning are in criterion 4 below and in `docs/PIPELINE_RULE_CHANGELOG.md`. Phases 0 to 3 and 6 landed first; Phase 4 promoted the restated bundle `urc-2024-25-v4-6f04bd64d2a6-a2` after Abdel's recorded yes on the diff summary; Phase 5 deleted 93 items (354.5 MB) per the manifest below with Abdel's approval. See `docs/PIPELINE_RULE_CHANGELOG.md`, entry "2026-07-24: 2024-25 lineage restatement released, and Phase 5 cleanup executed", for the released figures, the equivalence argument for the performance-only migration, and the recorded deviation on ledger-evidence marking. This document is retained as the plan of record; it is not a live task list.
...
- Frozen workbook vs successor v4 (`outputs/urc_final_human_review_2024-25/urc_injury_reviewed_master_with_exclusion_decisions_2024-25_frozen_2026-07-23.xlsx` vs `..._successor_2026-07-23_v4.xlsx`): exactly 180 value diffs, all accounted for by existing audit records (178 Sharks "Other team" to "Sharks"; row 535 date typo fix; row 1530 "Confirmed duplicate" exclusion plus red row font). No rows added or removed. Red/green cues are direct per-cell fonts (no conditional formatting anywhere) and are preserved cell-for-cell. The frozen workbook was moved to `data/2024-25/archive/review-workbooks/2026-07-23-frozen/` on 2026-08-24; this comparison remains historical evidence only.
...
4. **Amended 2026-07-25 (Abdel's decision; original wording below).** A single small edit (for example, adding a confirmed return date) flows through the data layer with one command (`python3 tools/replay.py`) plus one review (Abdel's recorded yes on the printed diff summary). Reaching the website is deliberately a separate, separately approved path: lineage load, `release-league` preflight, recorded approval, promotion, then `export-team-dashboards`.

   Original wording: "A single small edit (for example, adding a confirmed return date) flows to the website preview through one command plus one review." Why amended: a genuine single command from edit to website would have to perform a live database write and a release promotion without stopping, which the binding rule in `AGENTS.md` forbids (every live write and every promotion needs Abdel's explicit approval of the exact hosted target). The criterion was written before that boundary was settled, and the approval gates are worth more than the convenience. The one-command, one-review property is preserved exactly where it belongs, in the data layer, which is where the original intent (no ceremony for routine edits) actually applies.
