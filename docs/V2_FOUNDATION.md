...
The application now uses the season-aware V6 readers. Before the URL is shared or production cutover begins, execute the protected-access restoration gate in `AGENTS.md`, verify the least-privilege `web_reader` configuration, and smoke-test the dynamic team routes at desktop and mobile widths. Deployment, sharing and cutover remain separate approvals.
