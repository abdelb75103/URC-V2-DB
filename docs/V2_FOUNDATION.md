...
- **Rollout.** Every new team/season first passes the local, read-only `docs/TEAM_INTAKE_PROFILING_GATE.md`: inventory its schemas and source values, establish reporting windows and exposure grain, map its clinical categories and supporting evidence into the frozen canonical/IOC buckets, complete a fresh AI review, and obtain Abdel's approval. Only then do the remaining ten teams follow the frozen path (`ingest` → `process-intake`/`process-exposure` → `build-curated` → read-only release preflight → sign-off → release), one team at a time as their intake files arrive. An approved database release does not unlock a public route: the club remains inaccessible while `config/teams.ts` says `locked`; changing it to `live` and deploying are separate explicit manual approvals.

## Next Step

Profile and approve each of the remaining ten teams under `docs/TEAM_INTAKE_PROFILING_GATE.md`, then process it through the frozen DB-backed path as its pseudonymised intake files arrive, one team at a time with per-team sign-off. At cutover, put the `web_reader` credential into the Vercel project's env settings and confirm the production build prerenders from the database.
