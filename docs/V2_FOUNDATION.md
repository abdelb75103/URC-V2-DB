...
- **Rollout.** The remaining ten teams follow this frozen path (`ingest` → `process-intake`/`process-exposure` → `build-curated` → read-only release preflight → sign-off → release), one team at a time as their intake files arrive. An approved database release does not unlock a public route: the club remains inaccessible while `config/teams.ts` says `locked`; changing it to `live` and deploying are separate explicit manual approvals.
