...
Status: accepted foundation, operational reader status updated 27 July 2026
...
- The repository protects every `live` team dashboard with server-side password hashes and signed, expiring, HttpOnly, exact-team sessions. It has no Supabase Auth or admin interface.
- Before the V2 URL is shared with teams or the public, or before production cutover, provision the fresh hashes and signing key, confirm rate limiting and preview protection, then complete the browser checks in `docs/ACCESS_RESTORATION_GATE.md`.
- Commit `2cbfb6e` remains the reviewed restoration baseline. The current implementation keeps the later season-aware dashboard and report features inside that access boundary.
...
- Server-render approved league aggregates and password-protected live-team aggregates from reporting views.
...
- Before public production, provision the signed-session secrets, rate-limit unlock requests, and protect preview deployments. Treat every legacy password as public and use the fresh V2 passwords supplied on 16 September 2026.
...
The full lakehouse path is live and is the single source of truth for every published number: `ingestion` → `processing` (+ `audit`) → `curated` (typed builds) → versioned analysis views → reviewed per-team release snapshots → immutable league bundle → correction-aware unified bundle → `reporting.latest_team_dashboard_v6` / `reporting.latest_league_dashboard_v6` (the season-aware website consumer views). Their 2024-25 branch passes through the correction-aware V5 path. The frozen V2 storage remains the approved predecessor layer.
...
- **Bundle and website.** A per-team `pipeline release` preserves the reviewed member snapshot but does not change the site until `pipeline release-league` promotes a reviewed immutable 16-team bundle. Classification-only successors use versioned incremental candidates: the approved payload is copied and only `body_locations`, `injury_types`, and `injury_profiles` are replaced from the accepted classification views; unrelated metrics are neither recalculated nor overwritten. Dynamic row corrections use the separately reviewed additive path in `docs/DYNAMIC_ROW_CORRECTION_WORKFLOW.md`, with retained predecessors and append-only rollback. `lib/reporting.ts` queries `reporting.latest_team_dashboard_v6` and `reporting.latest_league_dashboard_v6` server-side through the `web_reader` role. Those season-aware views pass 2024-25 through the correction-aware V5 path and expose only complete approved V6 bundles for later seasons. Team pages are dynamic and require a signed exact-team session before any reporting query runs. A missing session, credential, database connection, or approved bundle fails closed. `content/reporting/*.json` remains an emergency artifact and parity record, but the app does not import it. Secrets live only in Git-ignored `.env.local` and the deployment environment.
...
- **Rollout.** All 16 teams completed the 2024-25 path and are marked `live` behind exact-team password access. Every new team/season follows `docs/PIPELINE_RUNBOOK.md`: local profile and injury/exposure preparation; separate checksum-matched registrations and processing runs; once-per-season fixture loading; curated build; reconciliation/parity; reviewed team release; and reviewed immutable league-bundle release. A database release, bundle promotion, route status, access-control change, deployment, sharing, and production cutover remain distinct approval boundaries.

## Current open gate

The application now uses the season-aware V6 readers behind protected team routes. Before the URL is shared or production cutover begins, provision the access secrets and WAF rule, verify the least-privilege `web_reader` configuration, and smoke-test the routes at desktop and mobile widths. Deployment, sharing and cutover remain separate approvals.
