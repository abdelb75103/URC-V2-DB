...
test("reporting queries obtain database attestation before each reader SQL statement", async () => {
  ...
  assert.match(reporting, /reporting\.approved_dashboard_reader_target_v6/);
  assert.match(reporting, /target_attested/);
  assert.doesNotMatch(reporting, /pool\.query/);
  assert.match(reporting, /pool\.connect\(\)[\s\S]*begin transaction read only[\s\S]*approved_dashboard_reader_target_v6[\s\S]*client\.query<any, any\[]>\(sql, values\)/);
});
...
test("attestation and dashboard SQL run on one read-only database session", async () => {
  ...
  try {
    ...
    assert.match(statements[1], /approved_dashboard_reader_target_v6/);
