...
export function buildSeasonTimelineRows(rows: readonly MonthlySettingRow[], allowFallback = true): MonthlySettingRow[] {
  return rows.map((row) => ({
    ...row,
    ...
    incidence_per_1000h: monthlyIncidence(row.time_loss_injuries, row.exposure_hours, row.incidence_per_1000h, allowFallback),
  }));
}
