...
export function buildReportModel(request: ReportModelRequest): ReportModel {
  ...
  const model: ReportModel = {
    ...
    monthlyInjuryPattern: sortByMonth(current.monthly).map((row: AnalyticsRow) => ({
      ...
      timeLossInjuries: row.time_loss_injuries,
      daysLost: row.days_lost,
      overallIncidencePer1000h: monthlyIncidence(row.recorded_injuries, row.exposure_hours, row.overall_incidence_per_1000h, current.scope !== 'league'),
      incidencePer1000h: monthlyIncidence(row.time_loss_injuries, row.exposure_hours, row.incidence_per_1000h, current.scope !== 'league'),
    ...
    exposure: {
      ...
      totalHsrDistanceKm: current.coverage.hsr_distance_km ?? null,
      totalHsrPercentage: current.coverage.hsr_percentage ?? null,
      ...
      monthly: sortByMonth(current.monthly).map((row) => ({
        ...
        hsrDistanceKm: row.hsr_distance_km ?? null,
        hsrPercentage: row.hsr_percentage ?? null,
        isImputed: row.is_imputed ?? false,
        imputationMethod: row.imputation_method ?? null,
        displayNote: row.display_note ?? null,
        hsrSourceStatus: row.hsr_source_status ?? null,
      })),
    },
