...
- The 2024-25 curated layer contains July and August exposure rows, but every one is `excluded_from_primary` with `outside_official_analysis_window`, so the approved league exposure series begins in September. **Do not describe those months as unreported or add them to an approved payload.** For Abdel's approved private local review they may be shown through the development-only aggregate preview boundary so he can adjudicate them before any versioned rule change.
