...
- `release` (including preflight) refuses a dirty Git tree. Commit implementation changes first.
...
- The currently served V4 2024-25 exposure series begins in September. Historical July and August curated rows remain `excluded_from_primary` with `outside_official_analysis_window`; they are not unreported. The accepted, not-yet-promoted V5 successor uses the inclusive 1 September 2024 to 30 June 2025 cohort. It may re-admit only rows whose sole historical blocker is that window reason and whose native period overlaps V5, while preserving every other exclusion. The 1 to 19 September semantic non-URC-match rule applies only to the newly opened pre-URC band. Do not mutate curated rows, rebuild exposure, or generalise that rule outside its recorded scope. See `docs/evidence/analysis_window_2024-25_v5.json`.
