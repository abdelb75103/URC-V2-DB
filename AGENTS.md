...
`immutable intake files -> ingestion (source_files, source_rows) -> processing (record_versions) -> curated (builds, injuries, exposure, fixtures, code_lists, team_exposure_denominators) -> versioned analysis views -> reviewed team releases -> immutable league bundle -> correction-aware unified bundle -> reporting.latest_team_dashboard_v5 / reporting.latest_league_dashboard_v5 -> website`
...
| **Inclusion** | `data/2024-25/inclusion/urc_injury_included_dataset_2024-25.csv` | 2,309 rows x 28 columns, SHA-256 `7203b83954becb1c2232ff7e7efa73eac1da41d7533afce865fa325041d74d71`, retained source-row mapping SHA-256 `5409e641ad5d9b0159a94fc141899b1345149e5d3220cb734ca7a8da2c6ae470`. Generated by replay; analysis consumes only this layer. Never add headline metrics to it. |
...
- All of `data/` is Git-ignored. `content/reporting/*.json` is an emergency/parity export and public payload, **not an application input**: no internal `source_files`, `pipeline_evidence`, hashes, or audit paths. `lib/reporting.ts` queries `reporting.latest_team_dashboard_v5` and `reporting.latest_league_dashboard_v5` server-side via the least-privilege `web_reader` role (`WEB_READER_DB_URL`). Live team pages are dynamic and currently need no password; a missing credential, database error, or missing approved bundle fails closed to the unavailable shell.
...
- The dynamic correction path is documented in `docs/DYNAMIC_ROW_CORRECTION_WORKFLOW.md`. It corrects only an existing injury row through an append-only overlay, then separately promotes one affected team plus the pooled league result. The current release and rollback tables deliberately require a 16-team bundle. A replacement club within a 16-team season is supported after normal intake and release; a 17-team or larger season requires an additive member-count successor before correction promotion.
- The application queries only the V5 reader views. Older V2, V3 and V4 aggregate reader grants remain temporarily available for deployment rollback; their underlying views also have historical dependants and must not be dropped. Retire only the old `web_reader` grants through an additive migration after the V5 deployment and rollback window are verified.
...
- Before the first real dynamic correction, decide and record how its database adjudication is reconciled with the 2024-25 master, decision ledger and inclusion CSV. The correction tooling does not silently rewrite those lineage artifacts.
