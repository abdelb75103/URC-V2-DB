...
- **V3 scope split (decision recorded 19 July 2026; cohort implemented 20 July 2026):** migration `20260720170000` promotes only the accepted parameterised season-bound cohort. Injuries inside the registered season window are included without a team exposure-window restriction; season-attributed undated injuries enter counts/breakdowns but not monthly plots; exposure and fixtures use the same registered window. The broader inference/diagnosis layer in `tools/sql/dashboard_v3_preview.sql` remains dev-only and hard-off in production. Live use still requires an immutable V3 bundle release carrying the cohort-evidence checksum.
