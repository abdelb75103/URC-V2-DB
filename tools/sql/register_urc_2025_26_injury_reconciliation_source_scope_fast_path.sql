-- Register only the reviewed source-scoped reconciliation successor.
...
begin
  ...
  then
    raise exception 'URC 2025-26 source-scoped reconciliation objects are incomplete';
  end if;
  ...
  if position('injury_source_rows' in reconciliation_definition) = 0
    or position('join injury_source_rows source' in reconciliation_definition) = 0
  ...
  then
    raise exception 'URC 2025-26 reconciliation is not source-scoped';
  end if;
  ...
  then
    raise exception 'URC 2025-26 source-scoped reconciliation is not least-privilege';
  end if;
end;
$$;
...
values (
  '20260823185000',
  'urc_2025_26_injury_reconciliation_source_scope_fast_path',
  array['migration_sha256=6ecb9474f6274295b588ffde362904885e35d7fbd55ad52e3a19d38f2a9280a4']
)
...
begin
  if not exists (
    ...
    where migration.version = '20260823185000'
      and migration.name =
        'urc_2025_26_injury_reconciliation_source_scope_fast_path'
      and migration.statements = array[
        'migration_sha256=6ecb9474f6274295b588ffde362904885e35d7fbd55ad52e3a19d38f2a9280a4'
      ]
  ) then
    raise exception 'URC 2025-26 source-scoped reconciliation registration is absent or checksum-mismatched';
  end if;
end;
$$;
