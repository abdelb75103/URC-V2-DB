-- Register only the reviewed aggregate reconciliation successor.
...
begin
  ...
  then
    raise exception 'URC 2025-26 aggregate reconciliation objects are incomplete';
  end if;
  ...
  if position('processing_counts' in reconciliation_definition) = 0
    or position('eligible_counts' in reconciliation_definition) = 0
    or position('recorded_counts' in reconciliation_definition) = 0
    or position('from processing_eligible_injury row_set' in reconciliation_definition) <> 0
  then
    raise exception 'URC 2025-26 reconciliation still uses per-active source scans';
  end if;
  ...
  then
    raise exception 'URC 2025-26 aggregate reconciliation is not least-privilege';
  end if;
end;
$$;
...
values (
  '20260823180000',
  'urc_2025_26_injury_reconciliation_aggregate_fast_path',
  array['migration_sha256=4f9e17eaebb848040955678078f0d9e30faa128509f95df9d969e4f15636e116']
)
...
begin
  if not exists (
    ...
    where migration.version = '20260823180000'
      and migration.name = 'urc_2025_26_injury_reconciliation_aggregate_fast_path'
      and migration.statements = array[
        'migration_sha256=4f9e17eaebb848040955678078f0d9e30faa128509f95df9d969e4f15636e116'
      ]
  ) then
    raise exception 'URC 2025-26 aggregate reconciliation registration is absent or checksum-mismatched';
  end if;
end;
$$;
