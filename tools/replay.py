...
def _reporting_cohort_v5_section() -> list[str]:
    """Return the versioned reporting-cohort note for the 2024-25 methodology.

    The inclusion lineage is intentionally independent from this view-layer rule.
    Keeping the note here makes regeneration safe: a normal methodology refresh
    cannot silently erase the accepted distinction between inclusion and v5
...
def write_methodology(ledger: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(_reporting_cohort_v5_section())
...
def parse_args() -> argparse.Namespace:
    ...
    methodology_group = parser.add_mutually_exclusive_group()
    methodology_group.add_argument("--write-methodology", action="store_true")
    methodology_group.add_argument(
        "--write-methodology-only",
        action="store_true",
        help="regenerate methodology without replaying or writing the inclusion CSV or manifest",
    )
...
def main() -> int:
    args = parse_args()
    if args.write_methodology_only:
